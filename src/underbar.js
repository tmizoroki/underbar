(function() {
  ...
  _.last = function(array, n) {
    return n === undefined ? array[array.length - 1] : n === 0 ? [] : array.slice(-n); //Maybe refactor?
  };
  ...
  _.reduce = function(collection, iterator, accumulator) {
    var keys = _.map(collection, function(_, keys) {
      return keys;
    });
    var memo = (accumulator !== undefined) ? accumulator : collection[keys.shift()];
    _.each(keys, function(item) {
      memo = iterator(memo, collection[item]);
    });
    return memo;
  };
  ...
  _.extend = function(obj) {
    ...
    _.each(args, function(objToAdd) {
      _.each(objToAdd, function(val, key) {
        obj[key] = val;
      });
    });
  ...
  _.memoize = function(func) {
    ...
    return function() {
      ...
      return resultList[_.indexOf(argumentList, args.join(","))];
    };
  };
  ...
  _.zip = function() {
    var unzipped = Array.prototype.slice.call(arguments);
    var zipped = [];
    var i;

    var hasMoreToZip = function(counter) {
      return _.some(unzipped, function(array) {
        return array[counter] !== undefined;
      });
    };
